...
@testset "Integrator Fastkick" begin
    ###### Float64 ######
    ...
    s0 = State(ic)
    pair_kick = ones(Bool,s0.n,s0.n) ; pair_kick[1,:] .= false; pair_kick[:,1] .= false
    s0.pair .= pair_kick
    perturb!(s0)
    ...
    println("jac_step vs. jac_step_num: ",maximum(abs.(asinh.(s0.jac_step) .- asinh.(jac_step_num))))
    println("jac_step: ",s0.jac_step)
    println("jac_step_num: ",convert.(Float64,jac_step_num))
    ...
    s_nograd = State(ic)
    s_grad = State(ic)
    s_nograd.pair .= pair_kick
    s_grad.pair .= pair_kick
    tmax = 2000.0
