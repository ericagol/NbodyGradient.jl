...
@testset "Transit Timing Fastkicks" begin
    N = 3
    ...
    function calc_times(h, ti)
        ...
        s = State(ic)
        pair_kick = ones(Bool,s.n,s.n); pair_kick[1,:] .= false; pair_kick[:,1] .= false
        s.pair .= pair_kick
    ...
    function calc_finite_diff(h, t0, itime, tmax, elements, ti)
        ...
        sp, dp = dState(ic_big)
        sm, dm = dState(ic_big)
        pair_kick = ones(Bool,sp.n,sp.n); pair_kick[1,:] .= false; pair_kick[:,1] .= false
        sp.pair .= pair_kick
        sm.pair .= pair_kick
    ...
    pair_kick = ones(Bool,s_nograd.n,s_nograd.n); pair_kick[1,:] .= false; pair_kick[:,1] .= false
